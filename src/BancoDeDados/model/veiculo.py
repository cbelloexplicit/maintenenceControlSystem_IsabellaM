from model.empresa import Empresa

class Veiculo:
    def __init__(self, id_veiculo:str, placa:str, empresa:Empresa = None):
        self.__id_veiculo = id_veiculo
        self.__placa = placa
        self.__empresa = empresa

    # Getters
    def get_id_veiculo(self) -> str:
        return self.__id_veiculo
    
    def get_placa(self) -> str:
        return self.__placa

    def get_empresa(self) -> Empresa:
        return self.__empresa

    # Setters
    def set_id_veiculo(self, id_veiculo:str):
        self.__id_veiculo = id_veiculo

    def set_placa(self, placa:str):
        self.__placa = placa

    def set_empresa(self, empresa:Empresa):
        self.__empresa = empresa

    # To String
    def to_string(self) -> str:
        empresa_str = self.get_empresa().get_nome_fantasia() if self.get_empresa() else "N/A"
        return f"ID: {self.get_id_veiculo()} | Placa: {self.get_placa()} | Empresa: {empresa_str}"