...
BEGIN
  ...
  SELECT id_catalogo_defeito INTO V_ID_CAT_DEFEITO FROM CATALOGO_DEFEITOS WHERE codigo_defeito = 'GPS_SEM_SINAL';
  SELECT id_catalogo_acao INTO V_ID_CAT_ACAO FROM CATALOGO_ACOES WHERE codigo_acao = 'REINICIO_DISPOSITIVO';
  SELECT id_lot INTO V_ID_LOT FROM LOTS WHERE codigo_lot = 'LOT1001'; -- Supondo que este lote esteja no veículo
  ...
  -- 3. cria uma manutenca
  INSERT INTO MANUTENCOES (id_manutencao, id_visita, id_catalogo_acao, id_defeito, obs_servico)
  VALUES (SEQ_MANUTENCOES.NEXTVAL, V_ID_VISITA, V_ID_CAT_ACAO, V_ID_DEFEITO, 'Realizado reinício remoto do dispositivo.')
  ...
  SELECT id_catalogo_eventos INTO V_ID_CAT_EVENTO FROM CATALOGO_EVENTOS WHERE codigo_evento = 'MANUTENCAO_SEDE';
...
-- Salva todas as alterações dos blocos no banco
COMMIT;